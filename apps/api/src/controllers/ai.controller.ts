...
/** Optional body for generate: a confirmed feature checklist + which doc types. */
...
// PRD + TRD are the default set generated by the project "Generate" action.
...
export async function generateDocuments(req: Request, res: Response): Promise<void> {
  const project = await projectForGeneration(req);
  const { features, docTypes } = req.body as z.infer<typeof generateSchema>;
  ...
  const documents = await Promise.all(
    generated.map((g) => saveGeneratedDoc(project.id, g.docType, g.result)),
  );

  res.status(201).json({
    documents,
    tokensUsed: generated.reduce((sum, g) => sum + g.result.tokensUsed, 0),
  });
}
